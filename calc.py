...
#Эффекты кнопок
def on_enter(event):
    event.widget["bg"] = "#F58888"   # подсветка при наведении

def on_leave(event):
    event.widget["bg"] = "black"     # восстановление цвета

def on_press(event):
    event.widget["bg"] = "#1E1E1E"    # эффект нажатия
...
# Окно
root = tk.Tk()
root.title("Калькулятор для домашки")
root.configure(bg="grey")

root.bind("<Key>", keypress)

# Поле ввода
entry = tk.Entry(root, width=20, font=('Arial', 18), borderwidth=3,
                 relief="solid", justify="right", fg="white", bg="#526016")
...
for (text, row, col) in buttons:
    ...
    btn = tk.Button(root, text=text, width=5, height=2,
                    ...
                    relief="raised", bd=4, command=command)
